...
class ShapesDataset(Dataset):
    def __init__(self, root, transform=None):
        ...
        for cls_idx, cls_name in enumerate(classes):
            img_dir = self.root / cls_name / "images"
            meta_dir = self.root / cls_name / "labels"
            ...
            for img_path in sorted(img_dir.glob("*.png")):
                ...
                if labels_path.exists():
                    self.images.append((img_path, labels_path, cls_idx))

    def __len__(self):
        return len(self.images)

    def __getitem__(self, idx):
        img_path, labels_path, cls_idx = self.images[idx]
        ...
        data = labels_path.read_text().split()
        bbox = torch.tensor([float(x) for x in data[1:]], dtype=torch.float32)

        return tensor, cls_idx, bbox
...
epochs = 35
save_path = Path(__file__).parent / "best.pt"
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for images, cls_t, bbox_t in val_loader:
                ...
                pred_size = bbox_pred[:, 2:]
                true_size = bbox_t[:, 2:]
                ratio = pred_size / (true_size + 1e-7)
                size_diffs.extend(ratio.cpu().numpy().flatten())

        val_acc = correct / total
        avg_size_ratio = np.mean(size_diffs)
        ...
        size_condition = avg_size_ratio < 1.05
        ...
        if epoch % 5 == 0 or epoch == 1 or val_acc > best_acc - 0.01:
            print(
                ...
                f"acc={val_acc:.3f} "
                f"size_ratio={avg_size_ratio:.3f}"
            )

        if (val_acc >= accuracy_threshold and size_condition) or no_improve >= no_improve_count:
            break
